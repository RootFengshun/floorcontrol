...
class PendingReq(State):
    ...
    def enter(self):
        ...
        self.machine.nodeHandler.req_self_time = float(self.env.now)
        send(signal.FLOOR_REQUEST+' '+str(self.env.now) , self.id, self.env)
...
class Node:
    ...
    def recv_process(self):
        while(True):
            ...
            cur_signal = Node.last_signal.split(" ")[0]
            ...
            if cmp(cur_signal, signal.FLOOR_REQUEST) == 0:
                ...
                if cmp(self.sm.currentState.name, "state_idle") == 0:
                    ...
                    other_req_time = float(Node.last_signal.split(" ")[1])
                    if(self.req_self_time > other_req_time) :
